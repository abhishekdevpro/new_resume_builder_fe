...
const Template4 = () => {
    ...
    return (
        <div className="flex flex-col md:flex-row max-w-[8.5in] mx-auto my-5 bg-white shadow-lg p-5 border border-gray-200 rounded-lg">
            <div className="flex-1 max-w-4xl mx-auto bg-white p-6 border border-gray-300 rounded-lg shadow-sm">
                <h1 className="text-3xl font-bold text-blue-500 mb-2" style={{ color: headerColor }}>{resumeData.name}</h1>
                <h2 className="text-xl text-gray-800 mb-4">{resumeData.position}</h2>
                <p className="mb-4 text-sm text-gray-600">{resumeData.summary}</p>

                <h3 className="text-lg font-semibold text-gray-800 mt-6 mb-2 border-b-2 pb-1 text-[#818cf8]" style={{ color: headerColor }}>Work Experience</h3>
                {resumeData.workExperience.map((item, index) => (
                    <div key={index} className="mb-6">
                        <h4 className="text-lg font-semibold text-gray-800 mb-1">{item.company}</h4>
                        ...
                        <Droppable droppableId={`WORK_EXPERIENCE_KEY_ACHIEVEMENT-${index}`} type="WORK_EXPERIENCE_KEY_ACHIEVEMENT">
                            {(provided) => (
                                <ul className="list-disc ul-padding content" {...provided.droppableProps} ref={provided.innerRef}>
                                    {typeof item.keyAchievements === "string" &&
                                        item.keyAchievements.split("\n").map((achievement, subIndex) => (
                                            <Draggable key={`${item.company}-${index}-${subIndex}`} draggableId={`WORK_EXPERIENCE_KEY_ACHIEVEMENT-${index}-${subIndex}`} index={subIndex}>
                                                {(provided, snapshot) => (
                                                    <li
                                                        ref={provided.innerRef}
                                                        {...provided.draggableProps}
                                                        {...provided.dragHandleProps}
                                                        className={`hover:scale-105 transition-transform duration-300 hover:outline-dashed hover:outline-2 hover:outline-gray-400 mb-2
                                                        ${snapshot.isDragging && "outline-dashed outline-2 outline-gray-400 bg-gray-200"}`}
                                                    >
            ...
            <div className="flex-1 p-6 bg-gray-100" style={{ backgroundColor: backgroundColorss }}>
                <div className="mb-6">
                    <h4 className="text-lg font-semibold text-[#818cf8] mb-2 border-b border-gray-300 pb-1" style={{ color: headerColor }}>Contact</h4>
                    <ContactInfo
                        mainclass="contact"
                        linkclass="inline-flex items-center gap-1"
                        teldata={resumeData.contactInformation}
                ...
                <div className="mb-6">
                    <h4 className="text-lg font-semibold text-[#818cf8] mb-2 border-b border-gray-300 pb-1" style={{ color: headerColor }}>Skills</h4>
                    <Droppable droppableId="skills" type="SKILLS">
                        {(provided) => (
                            <div {...provided.droppableProps} ref={provided.innerRef}>
                                {resumeData.skills.map((skill, index) => (
                                    <Draggable key={`SKILLS-${index}`} draggableId={`SKILLS-${index}`} index={index}>
                                        {(provided, snapshot) => (
                                            <div
                                                ...
                                                {...provided.draggableProps}
                                                {...provided.dragHandleProps}
                                                className={`hover:scale-105 transition-transform duration-300 mb-1 ${snapshot.isDragging && "outline-dashed outline-2 outline-gray-400 bg-gray-200"}`}
                                            >
                                                <Skills title={skill.title} skills={skill.skills} />
                                            </div>
                                        )}
                ...
                <div className="mb-6">
                    {resumeData.education.length > 0 && (
                        <div className="mb-1">
                            <h2 className="text-lg font-semibold mb-1 border-b-2 text-[#818cf8]" style={{ color: headerColor }}>
                                Education
                            </h2>
                            {resumeData.education.map((item, index) => (
                                <div key={index} className="mb-2">
                                    <p className="content i-bold">{item.degree}</p>
                                    <p className="content text-gray-700">{item.school}</p>
